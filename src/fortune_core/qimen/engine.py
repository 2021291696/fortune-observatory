"""奇门遁甲排盘引擎（mainline-cn-v1 规则集）。

从 skills/qimen-dunjia/scripts/qimen_cli.py 逐函数移植（剥离 CLI 文件 IO），
两实现由差分测试对拍锁定一致；改这里必须同步 skill 脚本并跑
tests/differential/test_qimen_skill_parity.py。

规则要点（详见 skills/qimen-dunjia/references/ruleset-mainline.md）：
- 冬至到芒种阳遁、夏至到大雪阴遁，按当前节令判定；
- 三元按日干支在六十甲子中的位置每 5 日一元循环；
- 局数按节令×三元查固定定局表；
- 中宫相关判断一律寄坤处理；
- 天盘以旬首所遁之仪（代表甲）为起排参照，旬首落中宫时寄坤取坤宫地盘干。
"""
...
def resolve_timezone(location: dict[str, Any], warnings: list[str]) -> str:
    timezone_name = (location.get("timezone") or "").strip()
    if timezone_name:
...
def normalize_input(payload: dict[str, Any]) -> NormalizedInput:
    ...
    timezone_name = resolve_timezone(location, warnings)
    try:
