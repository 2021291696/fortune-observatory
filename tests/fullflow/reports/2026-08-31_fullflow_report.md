...
## 部署状态（2026-08-31 晚间更新：已全部闭环）

- ✅ 静态托管：常驻视图 + 对照命盘下线后的前端已上线
- ✅ 云函数 destiny-api：新口径包已上线（解法：本地给 npx 缓存 cli.js 的 COS 60 秒超时打补丁到 600 秒）
- ✅ 部署后线上验收：overlay 请求 422、解梦 19.8s/978 字正文干净、转介 ✓、运势(前端结构) 22.2s/921 字、命盘 14.4s
- ⚠️ 口径变大引发超时回归（22.4s 稳定 502），已修：默认超时 22→40s、硬顶 28→55s、解梦独立 50s（commit `23954a1`）
