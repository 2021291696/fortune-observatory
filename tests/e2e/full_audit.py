...
def main() -> None:
    with sync_playwright() as p:
        ...
        for label in ["健康", "姻缘", "事业", "财运"]:
            ...
            page.locator("#analysis .ai-answer").first.wait_for(timeout=15_000)
            page.locator("#analysis .ai-answer").nth(1).wait_for(timeout=15_000)
        check("问事.四板块AI全部生成", True)
        page.locator(".domain-choices button", has_text="事业").first.click()
        page.wait_for_timeout(600)
        check("问事.板块缓存复用秒显", page.locator("#analysis .ai-answer").count() == 2)
        page.get_by_role("button", name="换个问题追问 AI").first.click()
        page.locator("#analysis .ai-question textarea").first.fill("追问：我最该避免什么？")
        page.get_by_role("button", name="按新问题重新生成").first.click()
        ...
        page.locator('select[aria-label="出生年"]').select_option("1965")
        page.locator('select[aria-label="出生月"]').select_option("3")
        page.locator('select[aria-label="出生日"]').select_option("8")
        page.locator('select[aria-label="出生时（24 小时制）"]').select_option("6")
        page.locator('select[aria-label="出生分"]').select_option("10")
